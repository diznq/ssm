...
#define ID_LEN 32
	...
	void RPCOnRead(InVal fd, short ev, void *arg) {
#ifdef USE_LIB_EVENT
		SimpleLock lock(rpcMutex);
		Client *cl = (Client*)arg;
		if (!cl) return;
		int n = recv((SOCKET)fd, (char*)cl->buffer + cl->received, cl->expect, 0);
		if (n <= 0) {
	...
	void RPCOnAccept(InVal fd, short ev, void *arg) {
#ifdef USE_LIB_EVENT
		SimpleLock lock(rpcMutex);
		sockaddr_in ca;
		int sl = sizeof(ca);
		SOCKET client = accept((SOCKET)fd, (sockaddr*)&ca, &sl);
		if (client != -1) {
